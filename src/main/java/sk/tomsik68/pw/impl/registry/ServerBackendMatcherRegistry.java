...
public class ServerBackendMatcherRegistry extends BaseRegistry<IServerBackendMatcher> {
	...
	public IServerBackend getBackend(Server server) {
		...
		if (matching.size() < 1) {
			...
			if (matching.size() > 1) {
				ProperWeather.log.warning("Multiple backends for your server were found. Please check if the correct one was chosen. If not, please change your configuration. Available backends: " + Util.dumpArray(matching.toArray()));
			}
