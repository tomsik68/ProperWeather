...
public class WeatherDefinition{
	...
	public Color getColor(String key) {
		...
		if (this.cs.isList(key)) {
			...
			result = new Color(((Integer) list.get(0)).intValue(),
		...
		if (this.cs.isString(key)) {
			try {
				result = (Color) Color.class.getField(this.cs.getString(key))
						.get(Color.BLUE);
				if (result == null)
					throw new NullPointerException();
			} catch (Exception e) {
				System.out
						.println("[ProperWeather] Invalid WeatherDefinition color: "
								+ this.cs.getString(key));
			}
		}
		return result;
	}
