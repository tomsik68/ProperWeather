...
public class ConfigFile {
	...
	public EPermissions getPerms() {
		try {
			return EPermissions.valueOf(this.config.getString(
					"default-permissions", "OP").toUpperCase());
		} catch (Exception e) {
	...
	public static void generateDefaultConfig(File file) {
		try {
			file.createNewFile();
			System.out
					.println("[ProperWeather] Couldn't find configuration file. Creating a new one...");
			...
			pw.println(NODE_LOCFILE+": en.txt");
			pw.println("#--------------------------------------------#");
			...
			sb = sb.deleteCharAt(sb.length() - 1);
			pw.println("# Possible values(must exactly match): "
					+ sb.toString());
			...
			pw.close();
			System.out
					.println("[ProperWeather] Configuration file created at: "
							+ file.getAbsolutePath());
		} catch (Exception e) {
			System.out
					.println("[ProperWeather] Configuration file creation error: ");
			e.printStackTrace();
		}
	}
	...
	public ChatColor[] getColorTheme() {
		...
		if (colors.length == 2)
			return new ChatColor[] {
					ChatColor.valueOf(colors[0].toUpperCase()),
					ChatColor.valueOf(colors[1].toUpperCase()) };
		throw new IllegalArgumentException(
				"[ProperWeather] Invalid property value: "
						+ this.config.getString("colors"));
	}
	...
	public RegionType getRegionType(String worldName) {
		if (!this.config.contains("region-type." + worldName)) {
			System.out
					.println("[ProperWeather] Please setup region type for world '"
							+ worldName
							+ "'. Defaulting to world-specific region.");
			this.config.set("region-type." + worldName, "WORLD");
			try {
				this.config.save(new File(ProperWeather.instance()
						.getDataFolder(), "config.yml"));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		return RegionType.valueOf(this.config.getString(
				"region-type." + worldName, "world").toUpperCase());
	}
